 ...
 * 获取配工机械数据
 */
public class TallyDetailMachineData extends JsonDataModel {
    /**
     * 日志标签前缀
     */
    private static final String LOG_TAG = "TallyDetailMachineData.";
    ...
    protected void onRequestSuccess(JSONObject jsonObject) throws JSONException {
        ...
        if (jsonArray != null) {
            ...
            Log.i(LOG_TAG + "onRequestSuccess", "get TallyDetailMachineData count is " + jsonArray.length());


            for (int i = 0; i < jsonArray.length(); i++) {

                JSONArray jsonMachine = jsonArray.getJSONArray(i);
                Log.i(LOG_TAG + "onRequestSuccess", "get TallyDetailMachineData count is " + jsonMachine.length());
                if (jsonMachine.length() > 1) {
//         复选框标志（0未选中1选中）+机械编号+司机编号+机械+司机+起始时间+终止时间+数量
                    Map<String,Object> map = new HashMap<String,Object>() ;
                    map.put("code_machine", jsonMachine.getString(0));
                    map.put("machine", jsonMachine.getString(1));
//
                    // 添加到列表
                    Log.i(LOG_TAG + "onRequestSuccess", "TallyDetailMachineData map list count is " + map.toString());
                    all.add(map);
                    Log.i(LOG_TAG + "onRequestSuccess", "TallyDetailMachineData  all list count is " + all.toString());
                }
            }

            Log.i(LOG_TAG + "onRequestSuccess", "TallyDetailMachineData count list count is " + all.size());
        }
    }
