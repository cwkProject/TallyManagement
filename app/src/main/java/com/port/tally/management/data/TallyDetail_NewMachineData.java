 ...
 */
public class TallyDetail_NewMachineData extends JsonDataModel {
    /**
     * 日志标签前缀
     */
    private static final String LOG_TAG = "TallyDetail_NewMachineData.";
...
//        dataMap.put("Gbno", searchContent2);
    ...
    protected void onRequestSuccess(JSONObject jsonObject) throws JSONException {
        ...
        if (jsonArray != null) {
            ...
            Log.i(LOG_TAG + "onRequestSuccess", "get TallyDetail_NewMachineData count is " + jsonArray.length());


            for (int i = 0; i < jsonArray.length(); i++) {

                JSONArray jsonMachine = jsonArray.getJSONArray(i);
                Log.i(LOG_TAG + "onRequestSuccess", "get TallyDetail_NewMachineData count is " + jsonMachine.length());
...
//         复选框标志（0未选中1选中）+机械编号+司机编号+机械+司机+起始时间+终止时间+数量
                    ...
                    // 添加到列表
                    Log.i(LOG_TAG + "onRequestSuccess", "TallyDetail_NewMachineData  map list count is " + map);
                    all.add(map);
                    Log.i(LOG_TAG + "onRequestSuccess", "TallyDetail_NewMachineData  all list count is " + all.toString());
                }
            }

            Log.i(LOG_TAG + "onRequestSuccess", "TallyDetail_NewMachineData list count is " + all.size());
        }
    }
