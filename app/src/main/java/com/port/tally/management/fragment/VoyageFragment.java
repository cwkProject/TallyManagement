...
public class VoyageFragment extends BaseCodeListFragment<Voyage> {
    ...
    @Override
    protected void onSetClickBind(final DataChangeObserver<Voyage> clickListener) {
        if (clickListener != null) {
            editText.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Voyage voyage = new Voyage();
                    voyage.setName(editText.getText().toString());
                    clickListener.notifyDataChange(voyage);
                }
            });
        }
    }
