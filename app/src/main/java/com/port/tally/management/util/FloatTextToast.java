...
/**
 * 浮动的文本显示�?根据�?��提供的View，可以把文本显示到该View的下�?
 * 可以设置显示的时间，多了该时间后自动消失。目前只支持纯文本{@link String}类型的显�? * 因为要计算显示文本的宽度�? * @author michael_li(飞雪无情)
...
public class FloatTextToast {
	...
	/**
	 * 生成�?��FloatTextToast
	 * @param context Activity 上下�?	 * @param targetView  目标View，浮动文本要显示在哪个View下面
	 * @param text 要显示的文本
	 * @param duration 浮动文本显示的时�?{@link #LENGTH_LONG} {@link #LENGTH_SHORT}
	 * @return �?��FloatTextToast，可以调用{@link #show()}显示
	 */
	...
	/**在Handler调用的show方法，主要为了等待{@link #mTargetView}的位*/
	...
	private  Rect getContentViewPos(int[] targetPos){
		...
		//状�?栏高�?		targetView.getWindowVisibleDisplayFrame(windowVisibleRect);
	...
	public static Dialog makeText(OnClickListener onClickListener,
			ImageView myImageView, String text, int lengthLong) {
